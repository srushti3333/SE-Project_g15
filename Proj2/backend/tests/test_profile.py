...
def test_update_profile_success(client, auth_header):
    ...
    update_data = {"email": "newemail@example.com", "bio": "Updated bio"}
...
def test_update_profile_partial_fields(client, auth_header):
    ...
    response = client.put(
        "/api/profile/me", json={"bio": "Just bio update"}, headers=auth_header
    )
    assert response.status_code == 200
    data = response.get_json()
...
def test_update_profile_invalid_email(client, auth_header):
    ...
    response = client.put(
        "/api/profile/me", json={"email": "not-an-email"}, headers=auth_header
    )
...
def test_update_profile_duplicate_email(client, auth_header):
    ...
    response = client.put(
        "/api/profile/me", json={"email": "other@example.com"}, headers=auth_header
    )
