...
def test_create_group(client, auth_header):
    ...
    data = {
        ...
        "nextOrderTime": (datetime.now(UTC) + timedelta(hours=1)).isoformat().replace("+00:00", "Z"),
        "maxMembers": 5
    }
...
def test_get_specific_group(client, auth_header):
    ...
    group_resp = client.post('/api/groups', json={
        ...
        "nextOrderTime": (datetime.now(UTC) + timedelta(hours=2)).isoformat().replace("+00:00", "Z"),
        "maxMembers": 10
    }, headers=auth_header)
...
def test_update_group(client, auth_header):
    ...
    response = client.put(f'/api/groups/{group_id}', json=updated_data, headers=auth_header)
    assert response.status_code == 200
...
def test_join_and_leave_group(client, auth_header):
    ...
    group_resp = client.post('/api/groups', json={
        ...
        "nextOrderTime": (datetime.now(UTC) + timedelta(hours=3)).isoformat().replace("+00:00", "Z")
    }, headers=auth_header)
    group_id = group_resp.get_json()["id"]
