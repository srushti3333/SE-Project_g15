...
class PollOption(db.Model):
    ...
    def to_dict(self):
        votes_count = PollVote.query.filter_by(poll_id=self.poll_id, option_id=self.id).count()
        return {
            'id': self.id,
            'text': self.text,
            'votes': votes_count
...
class PollVote(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint('poll_id', 'username', name='unique_poll_vote'),)